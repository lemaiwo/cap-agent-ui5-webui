...
export function mountIndex(app, mountPath, bootstrapUrl, log, dist = distPath()) {
  ...
  // Mirrors express.static's own behaviour for a mount root: a request for
  // "/chat/" (or "/chat/index.html") serves the page, but a bare "/chat"
  // 301-redirects to "/chat/" so the browser's document base ends up under the
  // mount path. Without this, every relative reference the served HTML makes
  // — the UI5 resource-root mapping, `fetch("agents.json")` — resolves against
  // the server root instead of the mount path. Express treats "/chat" and
  // "/chat/" as the same route (strict routing is off by default), so one
  // handler has to branch on the actual request URL rather than the route path.
  app.get(mountPath, (req, res) => {
    if (req.originalUrl.split("?")[0].endsWith("/")) return send(req, res)
